...
# 确认跳转时间
# step_to_epic_card_draw
def step_confirm_change_time(**kwargs):
    hwnd = utils.get_hwnd_from_kwargs(kwargs)
    next_step = step_to_epic_card_draw
...
def step_to_epic_card_draw(**kwargs):
    hwnd = utils.get_hwnd_from_kwargs(kwargs)
    next_step = step_change_epic_list
...
# 这一步比较长中间无法暂定，退出
# 下一步step_to_main
def step_change_epic_list(**kwargs):
    hwnd = utils.get_hwnd_from_kwargs(kwargs)
    ...
    if is_change_wish_list:
        ...
        is_success =wish_list.epic_list_run(hwnd,target_epic_list,target_full_list,gold_cards_map)
        if not is_success:
            return False,None
        # 只需要设置一次,成功后置为False
...
# 检查史诗抽卡颜色
# 目标界面：史诗抽卡界面
# 蓝，紫，金，继续史诗抽卡，返回True,step_epic_card_draw
...
def step_check_epic_result_card_color(**kwargs):
    ...
    log.logger.info("史诗抽卡抽颜色:%s",card_color.name)
...
def stop_work():
    ...
    if draw_count > 0:
        ...
        log.logger.info(f"史诗英雄卡池金卡概率:{epic_color_count_map[ce.CardColor.GOLD]}/{draw_count}={(full_color_count_map[ce.CardColor.GOLD]/draw_count):.2f}")
        log.logger.info(f"史诗英雄卡池紫卡概率:{epic_color_count_map[ce.CardColor.PURPLE]}/{draw_count}={(full_color_count_map[ce.CardColor.PURPLE]/draw_count):.2f}")
